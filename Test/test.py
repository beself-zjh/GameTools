...
class GameAssist:  
    def __init__(self, wdname):  
        ...
        # 主截图的左上角坐标和右下角坐标  
        self.scree_left_and_right_point = (275, 334, 1109, 891)
        # self.scree_left_and_right_point = (277, 337, 346, 404)
        # 小图标宽高  
        self.im_width_x = 69
        self.im_width_y = 69
    ...
    def screenshot(self):  
        ...
        for x in range(8):  
            ...
            for y in range(12):  
                # print("show",x, y)  
                # exit()  
                top = x * self.im_width_y
                left = y * self.im_width_x
                right = (y + 1) * self.im_width_x
                bottom = (x + 1) * self.im_width_y
    ...
    def clickAndSetZero(self, x1, y1, x2, y2):  
        ...
        # (299, 251, 768, 564)  
        # 原理：左上角图标中点 + 偏移量  
        p1_x = int(self.scree_left_and_right_point[0] + (y1 - 1)*self.im_width_x + (self.im_width_x / 2))  
        p1_y = int(self.scree_left_and_right_point[1] + (x1 - 1)*self.im_width_y + (self.im_width_y / 2))  
        
        p2_x = int(self.scree_left_and_right_point[0] + (y2 - 1)*self.im_width_x + (self.im_width_x / 2))  
        p2_y = int(self.scree_left_and_right_point[1] + (x2 - 1)*self.im_width_y + (self.im_width_y / 2))  
